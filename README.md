...
```tsx
<Input multiline rows={4} label="Message" placeholder="Write something..." />
```
...
```tsx
<Select options={largeList} searchable placeholder="Search and select..." />
```

### Clearable

```tsx
<Select options={options} value={value} onChange={setValue} clearable />
```
...
```tsx
<Radio.Group value={value} onChange={setValue} direction="horizontal">
...
<Modal
  ...
  footer={
    <>
      <Button outline color="secondary" onClick={() => setOpen(false)}>
        Cancel
      </Button>
...
```tsx
<Card header="User Profile" footer={<Button size="sm">Edit</Button>}>
...
```tsx
<Card hoverable>Hover me for a lift effect.</Card>
```
...
```tsx
<Card shadow>No border, floating shadow.</Card>
```
...
```tsx
<Breadcrumb items={items} separator={<ChevronRightIcon />} />
```
...
```tsx
<InputNumber currency prefix="$" precision={2} placeholder="0.00" />
```

### With spinner controls

```tsx
<InputNumber controls min={1} max={99} defaultValue={1} />
```
...
```tsx
<Popover title="Information" content={<p>Popover body content goes here.</p>}>
  <Button>Click me</Button>
</Popover>
```
...
```tsx
<Popover content="Quick info on hover" trigger="hover">
  <span>Hover me</span>
</Popover>
```
...
```tsx
<Popover title="Settings" content={<SettingsForm />} showCloseButton>
  <Button>Open</Button>
</Popover>
```
...
function App() {
  ...
  return (
    <>
      {contextHolder}
      <Button onClick={() => api.success({ message: "Saved!" })}>Save</Button>
    </>
  );
}
```
...
```tsx
<DatePicker value={date} onChange={setDate} clearable minDate={new Date()} />
```
...
```tsx
<TimePicker value={time} onChange={setTime} timeMode="HH:mm:ss" />
```
...
```tsx
<DateTimePicker value={dt} onChange={setDt} timeMode="HH:mm:ss" />
```
...
```tsx
<DateTimePicker value={dt} onChange={setDt} format="DD/MM/YYYY HH:mm" />
```
...
<ColorPicker value={gradient} onChange={setGradient} mode="gradient" />;
```

### Both modes (tab switcher)

```tsx
<ColorPicker value={value} onChange={setValue} mode="both" />
```
...
<ColorPicker
  ...
  trigger={({ value }) => (
    <div
      style={{ width: 32, height: 32, background: value, borderRadius: 4 }}
    />
  )}
/>
```
